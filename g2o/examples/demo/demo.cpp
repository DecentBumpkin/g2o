...
int main(int argc, const char* argv[]){
    ...
    Eigen::Vector3d p_nominal(10,10,10);
    ...
    g2o::SE3Quat T_il_nominal(q_nominal, p_nominal);
    g2o::SE3Quat T_il_actual(Eigen::Quaterniond(1,0,0,0), Eigen::Vector3d(0,0,1));
    ...
    for(int i = 0; i < 5; i++){
        ...
        v_se3_wi->setEstimate(T_wi_gt);
        optimizer.addVertex(v_se3_wi);
        v_se3_wi->setFixed(true);
        // v_se3_wi->setMarginalized(true);
    ...
    for (size_t i = 0; i < 2; i++ )
    {
        ...
        v_xyz_feat->setEstimate(xyz_w_gt); 
        optimizer.addVertex(v_xyz_feat); 
        feature_points_gt.emplace_back(xyz_w_gt, vertex_id);
        // v_xyz_feat->setFixed(true);
        // v_xyz_feat->setMarginalized(true);
    }
    /* add ternary edges */
    ...
    std::cout << "Nominal Imu->Lidar SE3: \n" << T_il_nominal <<'\n';
    std::cout<< "result: \n" << dynamic_cast<g2o::VertexSE3Expmap*>(optimizer.vertices().find(0)->second)->estimate().translation().transpose() << '\n';
    std::cout << "Actual Imu->Lidar SE3: \n" << T_il_actual <<'\n';  
}
